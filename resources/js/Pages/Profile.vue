<script setup>
    ...
    const form = useForm({
        ...
        profile_image: null,
        birth_date: new Date(props.account_owner.birth_date).toLocaleDateString('en-ZA').replaceAll('/', '-'),
    });
    ...
    function changePassword() {
        resetPassword.post(route('password.change', { user: props.user }), {
            ...
            preserveScroll: resetPassword.errors.old_password || resetPassword.errors.password,
...
<template>
    ...
    <FormCard>
        <CardTitle value="Register" />
        <form @submit.prevent="form.patch(route('profile.patch', { user }))">
            <!-- <div class="w-full rounded-sm  bg-cover bg-center bg-no-repeat items-center">
                            <div :style="profilePreview ? `background-image: url('${profilePreview}');` : ''"
                                ...
                                <div class="bg-white/90 rounded-full w-6 h-6 text-center ml-28 mt-4">
                                    ...
                                    <input type="file" id="upload_profile" @input="changed" hidden />
            ...
            <div v-if="user.access_type != 'admin'" class="grid w-full">
                <SecondaryInput v-model="form.first_name" label="First Name" :error="form.errors.first_name" />
                <SecondaryInput v-model="form.last_name" label="Last Name" :error="form.errors.last_name" />
                <SecondaryInput v-model="form.middle_name" label="Middle Name" :error="form.errors.middle_name" />
            </div>

            <div v-if="user.access_type != 'admin'" class="grid md:grid-cols-2 xs:flex-col gap-2 w-full">
                <SecondaryInput v-model="form.sex" label="Sex" type="select" :options="['male', 'female']"
                    :error="form.errors.sex" />
                <SecondaryInput v-model="form.birth_date" type="date" label="Date of Birth"
    ...
    <FormCard>
        <form @submit.prevent="changePassword">
            ...
            <SecondaryInput label="Old Password" v-model="resetPassword.old_password"
                :error="resetPassword.errors.old_password" />
            <SecondaryInput label="New Password" v-model="resetPassword.password"
                :error="resetPassword.errors.password" />
            <SecondaryInput label="Confirm Password" v-model="resetPassword.password_confirmation"
                :error="resetPassword.errors.password_confirmation" />

            <CardSubmitButton value="Update" :disabled="resetPassword.processing" />
        </form>
    </FormCard>
